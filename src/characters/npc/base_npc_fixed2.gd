...
func start_dialog():
    ...
    if game_manager and game_manager.has_node("DialogManager"):
        ...
        if dialog_manager and dialog_manager.has_method("show_dialog"):
            print("Found dialog manager via game manager")
            dialog_manager.show_dialog(self)
        else:
            print("DialogManager found but doesn't have show_dialog method!")
    else:
        print("Dialog manager not found under game manager!")
...
func choose_dialog_option(option_index):
    ...
    if dialog and option_index < dialog.options.size():
        ...
        if current_dialog_node == "exit" or get_current_dialog().options.size() == 0:
            ...
            if game_manager and game_manager.has_node("DialogManager"):
                var dialog_manager = game_manager.get_node("DialogManager")
                if dialog_manager and dialog_manager.has_method("end_dialog"):
