...
func _ready():
    print("Final Self-Contained Test Scene - Starting")
...
func _create_all_elements():
    ...
    for npc_name in npc_data.keys():
        var npc_data_info = npc_data[npc_name]
        _create_npc(npc_name, npc_data_info.position, npc_data_info.color)

func _create_npc(npc_name, position, color):
    var npc = ColorRect.new()
    npc.name = npc_name.replace(" ", "_") # Make sure name is valid
...
func _input(event):
    if event is InputEventMouseButton and event.pressed and event.button_index == BUTTON_LEFT:
        if not dialog_visible:
            ...
            for npc_name in npc_data.keys():
                var node_name = npc_name.replace(" ", "_")
                var npc = get_node_or_null(node_name)
                if npc and _is_point_in_rect(event.position, npc.rect_position, npc.rect_size):
                    _handle_npc_click(npc_name)
                    return
        else:
            # If dialog is visible, clicks outside the dialog panel close it
            var dialog_rect = Rect2(dialog_panel.rect_position, dialog_panel.rect_size)
            if not dialog_rect.has_point(event.position):
                _close_dialog()
...
func _handle_npc_click(npc_name):
    ...
    # Update status label
    status_label.text = current_verb + " " + npc_name
    
    # Handle interaction based on verb
    if current_verb == "Look at":
        status_label.text = "You see " + npc_name + ". " + npc_data[npc_name].description
    elif current_verb == "Talk to":
        _start_dialog(npc_name)
    elif current_verb == "Use":
        status_label.text = "You can't use " + npc_name + "."
    elif current_verb == "Pick up":
        status_label.text = "You can't pick up " + npc_name + "."

func _on_verb_selected(verb):
    current_verb = verb
    print("Selected verb: " + verb)
    status_label.text = verb + " (click on an NPC)"
...
func _close_dialog():
    ...
    current_npc = null
    status_label.text = "Dialog ended. Select a verb and click on an NPC."
