...
func _ready():
    print("Implementation Test Scene - Starting up")
    ...
    # Create game manager
    var game_manager = load("res://src/core/game/game_manager_new.gd").new()
...
func _create_npcs():
    # Create Concierge
    var concierge = load("res://src/characters/npc/concierge_new.gd").new()
    concierge.position = Vector2(300, 400)
    add_child(concierge)
    
    # Create Security Officer
    var security = load("res://src/characters/npc/security_officer_new.gd").new()
