...
func _ready():
    print("DEBUG TEST SCENE STARTING")
    ...
    # Create game manager
    game_manager = load("res://src/core/game/game_manager_debug.gd").new()
    ...
    print("DEBUG TEST SCENE SETUP COMPLETE")
...
func move_to(position):
    target_position = position
    is_moving = true
    print("Player moving to: " + str(position))
...
func _create_npcs():
    ...
    concierge.set_script(load("res://src/characters/npc/base_npc_debug.gd"))
    ...
    security.set_script(load("res://src/characters/npc/base_npc_debug.gd"))
...
func _add_debug_label():
    ...
    debug_label.rect_position = Vector2(10, 540)
    debug_label.rect_size = Vector2(1000, 20)
    debug_label.text = "Debug Mode Active - Check Console for Messages"
...
func _input(event):
    if event is InputEventMouseButton and event.pressed and event.button_index == BUTTON_LEFT:
        print("Click detected at: " + str(event.position))
