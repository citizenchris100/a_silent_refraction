extends Node2D
# Click Feedback System - Provides visual feedback for player clicks
# This implementation follows the contract defined by the mock in the component tests
...
# Visual constants (matching component test expectations)
...
func _process(delta):
	...
	var lines_to_remove = []
	for i in range(adjustment_lines.size() - 1, -1, -1):
		var line = adjustment_lines[i]
		var elapsed = current_time - line.creation_time
		if elapsed >= FEEDBACK_DURATION:
			lines_to_remove.append(i)
	
	for idx in lines_to_remove:
		adjustment_lines.remove(idx)
	
	# Trigger redraw if we have active elements
	if active_feedbacks.size() > 0 or adjustment_lines.size() > 0:
		update()
...
func create_feedback_node(feedback: FeedbackMarker) -> Node2D:
	# Create a node that will draw the feedback marker
	var node = Node2D.new()
	node.position = feedback.position
	# Don't try to load non-existent script
	
	# Store drawing data in the node
	node.set_meta("color", feedback.color)
	node.set_meta("size", MARKER_SIZE)
...
func _draw():
	...
	for line in adjustment_lines:
		var current_time = OS.get_ticks_msec() / 1000.0
		var elapsed = current_time - line.creation_time
		var alpha = 1.0 - (elapsed / FEEDBACK_DURATION)
		
		draw_line(line.from, line.to, Color(ADJUSTED_COLOR.r, ADJUSTED_COLOR.g, ADJUSTED_COLOR.b, alpha * 0.6), LINE_WIDTH)
		
		# Draw arrow head
		var direction = (line.to - line.from).normalized()
		var arrow_size = 8
		var angle = 0.5  # radians
		
		var arrow_point1 = line.to - direction.rotated(angle) * arrow_size
		var arrow_point2 = line.to - direction.rotated(-angle) * arrow_size
		
		draw_line(line.to, arrow_point1, Color(ADJUSTED_COLOR.r, ADJUSTED_COLOR.g, ADJUSTED_COLOR.b, alpha * 0.6), LINE_WIDTH)
		draw_line(line.to, arrow_point2, Color(ADJUSTED_COLOR.r, ADJUSTED_COLOR.g, ADJUSTED_COLOR.b, alpha * 0.6), LINE_WIDTH)
	
	# Draw feedback markers (circles)
	for feedback in active_feedbacks:
		if not feedback.node:
			# Fallback drawing if node doesn't exist
			draw_circle(feedback.position, MARKER_SIZE / 2, feedback.color)
